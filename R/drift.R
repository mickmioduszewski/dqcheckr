...
#' @param text_report Logical. Whether to also write a plain-text report
#'   alongside the HTML report. Defaults to \code{FALSE}.
...
compare_snapshots <- function(dataset_name,
  ...
  if (report) {
    ...
    if (text_report) {
      txt_path <- file.path(report_dir, paste0(base_name, ".txt"))
      .write_drift_text_report(drift, txt_path)
    }
  }
...
.compute_drift <- function(con, dataset_name, id_prev, id_curr, thresholds) {
  ...
  # Per-column stat drift — pivot long->wide per snapshot then merge
  ...
  # Helper: safely extract a column from the wide frame, returning NA if absent
  .col <- function(df, nm) {
  ...
  # Pre-filter and sort each section
...
.write_drift_text_report <- function(drift, outfile) {
  ...
  cat(sep, "\n", file = con)
  cat("DRIFT REPORT:", drift$dataset_name, "\n", file = con)
  cat(sep, "\n", file = con)
  cat(sprintf("Previous: snapshot #%d | %s | %s\n",
              drift$snap_prev$id, drift$snap_prev$run_timestamp,
              drift$snap_prev$file_name), file = con)
  cat(sprintf("Current:  snapshot #%d | %s | %s\n\n",
              drift$snap_curr$id, drift$snap_curr$run_timestamp,
              drift$snap_curr$file_name), file = con)

  cat("--- TABLE-LEVEL DRIFT ---\n", file = con)
  for (i in seq_len(nrow(drift$table_drift))) {
    r    <- drift$table_drift[i, ]
    flag <- if (r$Exceeds == "***") " *** EXCEEDS THRESHOLD" else ""
    cat(sprintf("  %-15s  %8s -> %8s  (%s)%s\n",
                r$Metric, as.character(r$Previous),
                as.character(r$Current), r$Change_pct, flag), file = con)
  }
  cat("\n", file = con)

  cat("--- SCHEMA DRIFT ---\n", file = con)
  if (nrow(drift$schema_changes) == 0) {
    cat("  No schema changes detected.\n", file = con)
  } else {
    for (i in seq_len(nrow(drift$schema_changes))) {
      r <- drift$schema_changes[i, ]
      detail <- if (r$Status == "TYPE CHANGED")
        sprintf("  (%s -> %s)", r$Type_Previous, r$Type_Current) else ""
      cat(sprintf("  %-30s  %s%s\n", r$Column, r$Status, detail), file = con)
    }
  }
  cat("\n", file = con)

  cat("--- PER-COLUMN DRIFT (*** = exceeds threshold) ---\n\n", file = con)

  mr <- drift$missing_rate_changes
  if (nrow(mr) > 0) {
    cat("  Missing Rate Changes (pp):\n", file = con)
    for (i in seq_len(nrow(mr))) {
      flag <- if (mr$missing_rate_exceeds[i]) " ***" else ""
      cat(sprintf("    %-30s  %+.2f pp  (%.1f%% -> %.1f%%)%s\n",
                  mr$Column[i], mr$missing_rate_change_pp[i],
                  mr$missing_rate_prev[i] * 100,
                  mr$missing_rate_curr[i] * 100, flag), file = con)
    }
    cat("\n", file = con)
  }
  ...
  if (nrow(nn) > 0) {
    cat("  Non-Numeric Rate Changes (pp):\n", file = con)
    for (i in seq_len(nrow(nn))) {
      flag <- if (nn$non_numeric_rate_exceeds[i]) " ***" else ""
      cat(sprintf("    %-30s  %+.2f pp  (%.2f%% -> %.2f%%)%s\n",
                  nn$Column[i], nn$non_numeric_rate_change_pp[i],
                  nn$non_numeric_rate_prev[i] * 100,
                  nn$non_numeric_rate_curr[i] * 100, flag), file = con)
    }
    cat("\n", file = con)
  }
  ...
  if (nrow(ms) > 0) {
    cat("  Numeric Mean Shifts:\n", file = con)
    for (i in seq_len(nrow(ms))) {
      flag <- if (ms$numeric_mean_exceeds[i]) " ***" else ""
      cat(sprintf("    %-30s  %+.1f%%  (%.4g -> %.4g)%s\n",
                  ms$Column[i], ms$numeric_mean_shift_pct[i] * 100,
                  ms$numeric_mean_prev[i], ms$numeric_mean_curr[i], flag),
          file = con)
    }
    cat("\n", file = con)
  }
  ...
  if (nrow(dc) > 0) {
    cat("  Distinct Count Changes:\n", file = con)
    for (i in seq_len(min(20L, nrow(dc)))) {
      pct <- if (!is.na(dc$distinct_count_change_pct[i]))
        sprintf("  %+.1f%%", dc$distinct_count_change_pct[i] * 100) else ""
      cat(sprintf("    %-30s  %+d  (%d -> %d)%s\n",
                  dc$Column[i], dc$distinct_count_change[i],
                  dc$distinct_count_prev[i],
                  dc$distinct_count_curr[i], pct), file = con)
    }
    if (nrow(dc) > 20L)
      cat(sprintf("    ... and %d more columns\n", nrow(dc) - 20L), file = con)
    cat("\n", file = con)
  }
