...
.rule_vocabulary <- function() {
  rbind(
    ...
    .rule_row("max_row_count_change_pct", "rules", "number >= 0 (fraction; 0.2 = 20%, values > 1 allowed)",
      .default_comparison_rules$max_row_count_change_pct,
      "Maximum tolerated row-count change vs the previous delivery, as a fraction (0.2 = 20%; values above 1 tolerate more than doubling)."),
    ...
    .rule_row("max_numeric_mean_shift_pct", "both", "number >= 0 (fraction; 0.2 = 20%, values > 1 allowed)",
      .default_comparison_rules$max_numeric_mean_shift_pct,
      "Maximum tolerated shift of a numeric column's mean vs previous, as a fraction (0.2 = 20%). Per-column value in column_rules wins over the rules-level one."),
