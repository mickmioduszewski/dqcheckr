...
**Version**: 0.2.1 **Author**: Mick Mioduszewski **Date**: 2026-05-31
...
### C.1 Package structure
    ...
    │   ├── checks_generic.R     # QC-01..16 and SC-01..02
    │   ├── checks_custom.R      # source and call custom_checks(df)
    │   ├── compare.R            # CP-01, CP-02a/b/c, CP-03..08
...
| QC-11 | Non-numeric values | Numeric columns | `non_numeric_rate > max_non_numeric_rate` | `non_numeric_rate > warn_non_numeric_rate` (default 0.0) |
| QC-12 | Key uniqueness | Configured `key_columns` (single or composite list) | Any duplicate in key column(s) | — |
...
| QC-14b | Maximum row count | Table | `row_count > max_row_count` (if configured) | — |
| QC-15 | File size | Table | `file_size_mb > max_file_size_mb` (if configured) | — (INFO otherwise) |
| QC-16 | Outlier detection | Numeric columns (if `max_z_score` configured) | `abs(Z) > max_z_score` per column | — |
...
| CP-02a | Schema diff: new columns | New columns detected (`flag_new_columns: false` suppresses) | WARN |
| CP-02b | Schema diff: dropped columns | Dropped columns detected (`flag_dropped_columns: false` suppresses) | WARN |
| CP-02c | Schema diff: type changes | Column type changed (`flag_type_changes: false` suppresses) | WARN |
| CP-03 | Missing rate change | Increase \> `max_missing_rate_change_pp` pp per column | WARN (or FAIL via `missing_rate_change_severity`) |
...
| CP-07 | Non-numeric rate change | Change \> `max_non_numeric_rate_change_pp` pp per column | WARN (PASS when rate does not increase) |
| CP-08 | Column order change | Column order differs from previous (`flag_column_order_change: false` skips; severity via `column_order_severity`) | WARN (CSV) / FAIL (FWF) by default |

> CP-02 is split into three independent result objects (CP-02a, CP-02b,
> CP-02c) so each dimension of schema change can be inspected and
> flag-suppressed separately. The `new_cols_vs_previous`,
> `missing_cols_vs_previous`, and `type_changed_cols_vs_previous`
> columns in the `snapshots` table are derived from these results.
...
— all logic is implemented natively.
