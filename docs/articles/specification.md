...
> `new_cols_vs_previous` and `missing_cols_vs_previous` columns in the
> `snapshots` table are derived directly from the CP-02 result — they
...
| `snapshot_db` path missing | Created automatically by [`init_snapshot_db()`](https://mickmioduszewski.github.io/dqcheckr/reference/init_snapshot_db.md) |
