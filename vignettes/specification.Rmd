...
| QC-16 | File encoding | File | Delivery is not valid UTF-8 when the config declares UTF-8 (whole file scanned before parsing; read completes via a single-byte fallback and the likely actual encoding is reported). Declared ASCII is read as UTF-8 (lossless subset); declared single-byte encodings always PASS (no invalid byte sequences exist) | — |
