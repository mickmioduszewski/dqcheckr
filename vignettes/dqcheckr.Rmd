...
All dependencies are on CRAN: `readr`, `stringi`, `DBI`, `RSQLite`, `quarto`,
`knitr`, `kableExtra`, `ggplot2`, `gridExtra`, `yaml`, `tidyr`, `rlang`.
Report rendering additionally needs the Quarto CLI installed separately (see
<https://quarto.org>).
...
or `current_file`), the `format`, and — for `format: fwf` — `fwf_widths`.
Every other config key is optional — the corresponding checks are simply
...
Some deliveries repeat column names in the header (e.g. a file that bundles
several record groups per row under identical labels: `PayeeName`, `Amount`,
`PayeeName`, `Amount`, …). A header like that cannot be used as-is. Supply
explicit `col_names` to name the columns positionally, and set `csv_skip: 1`
so the original header row is dropped rather than read as data:
...
`csv_skip` is the CSV analogue of `fwf_skip`; it defaults to `0`, so existing
configs are unaffected. For a genuinely headerless file, supply `col_names`
and leave `csv_skip` at `0`. (The `dqcheckrGUI` wizard sets both keys for you
when it detects duplicate or invalid header names.)
...
Every check function is exported and can be called directly, without going
through the full pipeline. All checks share the same signature —
...
The file is sourced in an isolated environment (not the global session) whose
parent is `baseenv()`. `dq_result()` is explicitly available without
qualification. All other dqcheckr functions (e.g. `resolve_col_type()`,
`infer_col_type()`) must be called with the package prefix:
`dqcheckr::resolve_col_type()`. Standard base R functions are fully available.
Any error — missing file, undefined function, runtime failure — stops the run
